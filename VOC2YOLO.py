...
def generate_train_and_val_txt(args):
    ...
    with open(target_train_file, 'w', encoding='utf-8') as f:
        ...
        for file in tqdm(train_images, desc=f"\033[1;33mProcessing Files for labels\033[0m"):
...
def parse_args():
    ...
    parser.add_argument('--train_list_name', type=str, default="labels.txt", help="训练图片列表名称")
    ...
    parser.add_argument('--classes', help="数据集具体类别数(用于生成 classes.json 文件)",
                        default=['aeroplane', 'bicycle', 'bird', 'boat', 'bottle', 'bus', 'car', 'cat',
                                 ...
                                 'pottedplant', 'sheep', 'sofa', 'labels', 'tvmonitor'])
...
def configure_path(args):
    ...
    # 生成对应的 labels.txt 和 val.txt
    generate_train_and_val_txt(args)

    # 检查文件/文件夹都是否存在
    assert os.path.exists(args.voc_images_path), f"VOC JPEGImages path not exist...({args.voc_images_path})"
    assert os.path.exists(args.voc_xml_path), f"VOC xml path not exist...({args.voc_xml_path})"
    assert os.path.exists(args.train_txt_path), f"VOC labels txt file not exist...({args.train_txt_path})"
...
def translate_info(file_names: list, save_root: str, class_dict: dict, train_val='labels', args=None):
    """
    ...
    save_images_path = os.path.join(save_root, train_val, "JPEGImages")
...
def main(args):
    ...
    translate_info(train_file_names, args.save_file_root, class_dict, "labels", args=args)
